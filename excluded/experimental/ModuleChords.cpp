...
uint32_t ModuleChords::compute()
{
	...
	this->note_1_output->value = NOTES[CHROMATIC[min((root_note_input + CHORDS[chord_input][0]),60)]];
	this->note_2_output->value = NOTES[CHROMATIC[min((root_note_input + CHORDS[chord_input][1]),60)]];
	this->note_3_output->value = NOTES[CHROMATIC[min((root_note_input + CHORDS[chord_input][2]),60)]];
